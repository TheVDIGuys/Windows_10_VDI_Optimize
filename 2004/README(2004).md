...
2. Uninstall the apps for "AllUsers", and optionally delete the payload.  The text input file "Win10_2004_AppxPackages.txt" uninstalls these apps by default.

3. Edit the default user registry hive, which these scripts do.  The REG.EXE commands have been recently added to the file "Win10_2004_DefaultUserSettings.txt" in this repository.  That way if you want to keep one or all of these apps, and still control the behavior, you can do so with the scripting method.
